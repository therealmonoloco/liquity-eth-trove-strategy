...
def test_direct_transfer_increments_estimated_total_assets(strategy, weth, weth_whale):
    initial = strategy.estimatedTotalAssets()
    amount = 100 * (10 ** weth.decimals())
    weth.transfer(strategy, amount, {"from": weth_whale})
...
def test_deposit_should_not_increment_profits(vault, strategy, weth, weth_whale):
    initialProfit = vault.strategies(strategy).dict()["totalGain"]
    assert initialProfit == 0

    weth.approve(vault, 2 ** 256 - 1, {"from": weth_whale})
    vault.deposit(1000 * (10 ** weth.decimals()), {"from": weth_whale})
